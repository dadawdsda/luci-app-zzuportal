...
2. 优先探测该设备到 `check_host` 的实际连通性；探测成功时本轮结束，不请求 Portal 状态接口。
3. 仅当连通性探测失败时，才通过 `info_url` 查询 Portal 登录状态。
...
9. 请求超时或无法识别的普通响应：记录错误并等待下一轮，不自动修改 MAC。
...
这些特征来自异常状态下对任意 HTTP 请求返回的强制重定向页面。只有明确命中特征时
才执行改 MAC，避免把普通断网误判为 Portal 状态混乱。
